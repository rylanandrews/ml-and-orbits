...
def assess(model, fromTraining, trainingDataset):
    ...
    initConditions = np.array(i).flatten()
    ...
    return (sortRawData(rCl, rMl), computeTimes, initConditions)
...
def main(numTrials, fromTraining=False):
    ...
    with open("outputs/sample_results.csv", 'w', newline='') as csvFile:
        csvWriter = csv.writer(csvFile)
        csvWriter.writerows(results)
    with open("outputs/sample_performances.csv", 'w', newline='') as csvFile:
        csvWriter = csv.writer(csvFile)
        csvWriter.writerows(performances)
    with open("outputs/initConditions.csv", 'w', newline='') as csvFile:
        csvWriter = csv.writer(csvFile)
        csvWriter.writerows(init)


main(numTrials=10, fromTraining=False)